...
impl BTreeNode {
    ...
    fn get_keys_from(&self, index: usize) -> Result<(Vec<Key>, Vec<Row>), Status> {
        PagerProxy::get_keys_encoded(&self).map(|v| (v.0[index..].to_vec(), v.1[index..].to_vec()))
    }
    ...
    fn get_key_encoded(&self, index: usize) -> Result<(Key, Row), Status> {
        PagerProxy::get_key_encoded(index, &self)
    }
    ...
    fn set_key_encoded(&self, index: usize, key: Key, row: Row) -> Result<(), Status> {
        PagerProxy::set_key_encoded(index, self, key, row)
    }
    ...
    fn set_keys_encoded(&self, keys: Vec<Key>, rows: Vec<Row>) -> Result<(), Status> {
        PagerProxy::set_keys_encoded(self, keys, rows)
    }
    ...
    fn get_keys_encoded(&self) -> Result<(Vec<Key>, Vec<Row>), Status> {
        PagerProxy::get_keys_encoded(self)
    }

    /// Removes a Key. The Children are JUST shifted to the left
    #[deprecated]
    fn remove_key(&self, index: usize) -> Result<(Key, Row), Status> {
        let mut keys_and_rows = self.get_keys_encoded()?.clone();
        let removed_key = keys_and_rows.0.remove(index);
        let removed_row = keys_and_rows.1.remove(index);
        PagerProxy::set_keys_encoded(self, keys_and_rows.0, keys_and_rows.1)?;
    ...
    fn push_key(&self, key: Key, row: Row) -> Result<(), Status> {
        let mut keys_and_rows = self.get_keys_encoded()?;
    ...
    fn extend_over_keys(&self, index_from: usize, index_to: usize) -> Result<(), Status> {
        let mut children = PagerProxy::get_children(self)?;
        let new_keys_and_rows = children[index_from].get_keys_encoded()?.clone();
...
impl Btree {
    pub fn init(
        ...
        if table_schema.btree_order == 0 {
            table_schema.btree_order = t;
        }
    ...
    fn insert_non_full(&self, x: &BTreeNode, key: Key, row: Row, t: usize) -> Result<(), Status> {
        ...
        if x.is_leaf() {
            ...
            if child_i.get_keys_count()? == (2 * t) - 1 {
                ...
                let key_and_row = x.get_key_encoded(i)?;
    ...
    fn split_child(&self, x: &BTreeNode, i: usize, t: usize, is_root: bool) -> Result<(), Status> {
        ...
        let key_and_row = y.get_key_encoded(t - 1).unwrap();
        //TODO suboptimal (works super fine)
        if is_root {
            x.set_key_encoded(0, key_and_row.0, key_and_row.1).unwrap();
        } else {
    ...
    fn delete_from(&self, x: &BTreeNode, k: Key, t: usize) -> Result<Option<BTreeNode>, Status> {
        ...
        while i < x.get_keys_count()?
            && self.compare(&k, &x.get_key_encoded(i)?.0)? == std::cmp::Ordering::Greater
        {
            i += 1;
        }

        if x.is_leaf() {
            if i < x.get_keys_count()? && k == x.get_key_encoded(i)?.0 {
    ...
    fn delete_internal_node(
        ...
        if x.get_child(i)?.get_keys_count()? >= t {
            ...
            x.set_key_encoded(i, pred_key_and_row.0.clone(), pred_key_and_row.1)?;
            ...
            x.set_key_encoded(i, succ_key_and_row.0.clone(), succ_key_and_row.1)?;
    ...
    fn get_predecessor(&self, x: &BTreeNode) -> Result<(Key, Row), Status> {
        ...
        cur.get_key_encoded(cur.get_keys_count()? - 1)
    }
    ...
    fn borrow_from_prev(&self, x: &BTreeNode, i: usize) -> Result<(), Status> {
        ...
        x.set_key_encoded(i - 1, last_sibling_key, last_sibling_row)?;
        if !child.is_leaf() {
    ...
    fn borrow_from_next(&self, x: &BTreeNode, i: usize) -> Result<(), Status> {
        let mut child = x.get_child(i)?;
        let mut sibling = x.get_child(i + 1)?;
        let k = x.get_key_encoded(i)?;
        child.push_key(k.0, k.1)?;
        let mut sibling_children = sibling.get_children()?;
        let sk = sibling.remove_key(0)?;

        x.set_key_encoded(i, sk.0, sk.1)?;
        if !child.is_leaf() {
